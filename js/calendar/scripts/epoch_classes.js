...
name nombre
    A unique name for the calendar – this will become the base for the calendar's id attribute. Un nombre único para el calendario - se convertirá en la base para la Identificación del atributo calendario. 
mode el modo de
    The mode the calendar will run in – can be flat or popup El modo en el calendario se ejecutará en - puede ser plano o emergente 
container contenedor
    A JavaScript resource pointing to the HTML element that will hold the calendar. Un recurso de JavaScript que apunta al elemento HTML que se mantenga el calendario. Usually accessed through the JavaScript method document.getElementById('container_id') . Por lo general, a través del método de JavaScript document.getElementById ('container_id'). 
multiselect selección múltiple
    Whether you can select multiple dates – true for yes, false for no. Ya sea que usted puede seleccionar varias fechas - cierto para sí, sin ninguna falsa. */
...
Epoch.prototype.setLang = function()  //all language settings for Epoch are made here.  Check Date.dateFormat() for the Date object's language settings
{
	...
	this.clearbtn_title = 'Limpia todas las fechas seleccionadas en el calendario';
	this.maxrange_caption = 'Este es el rango maximo';
};
//-----------------------------------------------------------------------------
