...
function cargaDatos(idDiv, idInput)
{
	...
	var divError=document.getElementById("error."+idInput);
	...
	else
	{
		...
		var ajax=nuevoAjax();
		var StrippedString = valorInput.replace(/(<\S([^>]+)>)/ig,"");
		ajax.open("GET", "ingreso_sin_recargar_proceso.php?dato="+StrippedString+"&actualizar="+idInput, true);
		ajax.send(null);
	}
}
...
function creaInput(idDiv, idInput)
{
	...
	if(!mostrandoInput && divError.style.display!="block")
	{
		...
		// Creamos el input
		divContenedor.innerHTML="<input type='text' onBlur='cargaDatos(\""+idDiv+"\", \""+idInput+"\")' value='"+divContenedor.innerHTML+"' id='"+idInput+"' maxlength='40' size='40'>";
