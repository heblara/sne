...
var com=ca.test(valor);
	...
	// Enviar la consulta
	    objeto.open("GET", "paginas/busquedaAdmin.php?opc="+sel+"&usuario=" + valor, true);
	    objeto.send(null);
	}
...
var com=ca.test(valor);
	if((!com=="") || (valor=="")){document.getElementById("resultado").innerHTML="";}else{
	// Enviar la consulta
	    objeto.open("GET", "paginas/busquedaAdmin.php?opc="+sel+"&usuario=" + valor + "&pagina="+pag, true);
	    objeto.send(null);
	}
...
<br />
<br />
<h1>BUSQUEDA DE NOTIFICACIONES</h1><hr /><br /><br />
<form name="buscar">
...
Texto a buscar:&nbsp;&nbsp;&nbsp;
<input type='search' id='txtDato' name='txtDato' onkeyup='leerDatos(this.value,0);' size="100" placeholder="Digite el texto a buscar (Asunto, Contenido o Cuenta Electrónica a la que se envió)" />
